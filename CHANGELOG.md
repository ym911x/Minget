...
已发布：[GitHub Release v1.4.2](https://github.com/ym911x/Minget/releases/tag/v1.4.2)。发布提交 `e094a67`、注释标签与 CI `35890064042` 成功均已核验。本机签名应用已安装并经用户确认三次重开正常。
...
- 同一 Profile 的完整读取、确认读取、唤醒探测共享一次进行中的结果；等待者收到当轮实际结果，忙碌不再被当作失败或缓存命中。
- 刷新进行中的手动点击排队恰好一次补刷；手动刷新按钮不再禁用。
- 瞬时失败立即重试一次，随后 30/60/120/300 秒自动退避（封顶 300 秒）；成功清零，手动重试旁路；退出登录、缺少 CLI 进入手动重试状态，不循环。退避门槛接入自动刷新调度器。
- 超时拆分：额度查询 15 秒、握手 5 秒、身份查询 3 秒；身份失败不阻止额度读取。
- 停止与取消以 shutdown 释放全部等待者，有界排空不等待卡死的操作。

### 身份显示

- 卡片身份三态：已确认、上次身份（橙色标签）、账号暂不可用；同一连接内的瞬时故障保留上次确认身份并明确标注。
- 额度成功但身份读取失败时降级为上次身份而不是清空；确认替换、退出登录等使旧身份失效。

### 卡片状态

- 卡片自带 fetching / live / cache / failure 与最近成功时间；详情页头部在存在未实时卡片时显示"部分数据未更新"。

### 点火

- 点火结果与重置时间观测继续分离：前移（≥60 秒观测）/ 未变化 / 未知；确认读取保持只读（2 秒、必要时 5 秒），无第二次模型请求。
- 点火后的确认读数以请求完成时间为新鲜度屏障，屏障前开始的共享读数不作为点火后证据。

### 外部点火计划脚本

- 新增 `scripts/minget-fire/` 模板：严格串行 A → 15 秒 → B → 15 秒 → Command Code，单请求 120 秒超时，`MINGET_FIRE_*` 环境覆盖全部路径与时限。
- Python 读取器改为字节缓冲 `os.read` + 单一单调总期限，兼容分包与粘包 JSON，自有进程组整组回收。
- 日志固定词表（target/start/end/exit/duration/window/used/reset/drift），观测行含前后 used 与差值；请求成功时观测未变化/不可用均退出 0，超时 124。
- 新增 `scripts/install-minget-fire.sh`：SHA256SUMS 哈希校验、时间戳备份并校验、临时文件原子替换、回滚指令；不写 plist、不重载 launchd、不从构建产物安装。
