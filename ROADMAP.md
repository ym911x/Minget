...
`v1.4.1` 外观优化已完成发布验收：详情页按 440 pt 单列重排，增加留白、两行服务头部、5/7 格与连续月度条、Command Code 两行统计，并新增四个本地显示名称入口。517 项自动测试执行，516 通过、1 项 AX 环境跳过、0 失败；Release arm64 构建、严格签名、签名包重启和真实界面验收通过。用户已确认按可见图案边界修订后的 OpenAI 与 DeepSeek Logo，并授权正式发布。资料见 `docs/versions/1.4.1/`。
