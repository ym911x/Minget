...
Codex 负责项目组织、架构、独立审核、质量验证和最终验收。Claude Code 默认负责编写应用代码、测试、构建脚本、实现说明和修订报告；用户明确指定 Codex 实现时，以当次指令为准。
...
4. 用模型请求验证任何 Key。除上述 Command Code 点火例外外，不得发起模型请求；应用网络层不得调用 chat/completions 或其他模型端点。
