...
`v1.4.2` 自动化测试、独立代码审核、arm64 严格签名安装、外部脚本备份安装及 2026-09-23 15:40 自然定时运行核验已完成。538 项 Swift 测试执行（Core 380、App 158），537 通过、1 项既有 AX 环境跳过、0 失败；外部假 CLI 与安装器 53 项断言通过。自然运行三路请求和 LaunchAgent 均 exit 0，但 A 窗口观测不可用、B 重置时间未变化。真实界面三次手动刷新已观察到数据返回；用户于 2026-09-24 确认详情页关闭重开三次均正常。第三轮聚合结束标志未单独留证，保留此证据边界。 公开发布核验及资料见 `docs/versions/1.4.2/`。

`v1.4.1` 外观优化已完成并发布：详情页按 440 pt 单列重排，增加留白、两行服务头部、5/7 格与连续月度条、Command Code 两行统计，并新增四个本地显示名称入口。517 项自动测试执行，516 通过、1 项 AX 环境跳过、0 失败；Release arm64 构建、严格签名、签名包重启、真实界面验收和发布提交 CI 通过。资料见 `docs/versions/1.4.1/`，发布页为 [GitHub Release v1.4.1](https://github.com/ym911x/Minget/releases/tag/v1.4.1)。
